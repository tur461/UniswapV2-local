...
async function main() {
  ...
  */
  ...
  const options = { value: token0Amount }
  ...
  .addLiquidityETH(
    ...
    const liq = await addLiquidityTx.wait();
    
    console.log('eth-gth liquidity added', routerArtifact.abi)
      const dec = (abi, data) => ethers.AbiCoder.defaultAbiCoder().decode(
        abi,
        data
    );
  const logs = liq.logs//.find(event => event.event === 'Transfer');
  const iface = new ethers.Interface(routerArtifact.abi);
  // const parsed = iface.parseTransaction({data: liq.data, value: liq.value});
  console.log(logs.map(log => log.topics[0].slice(0, 10)))

  const pEvs = pairArtifact.abi.filter(j => j.type == 'event');
  console.log('# of Event:', pEvs)
  
  console.log(dec([pEvs[5]], logs[1].data))
  ...
  console.log(`Total liquidity of Owner: ${liquidity}`)
  ...
  txn = await pair.approve(routerAddress, )
  await txn.wait()
